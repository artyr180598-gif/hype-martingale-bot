...
class MartingaleBot:
    ...
    def _restore_state(self):
        ...
        if not state or not state.get("in_trade"):
            return
        if self.demo_mode:
            logger.info("Демо‑режим: состояние не восстанавливается, сброс.")
            save_state({})
            return
        real_size = self.bybit.get_position_size(SYMBOL)
        if real_size == 0:
            logger.warning("Позиции на бирже нет — сбрасываем состояние")
            save_state({})
            return
    ...
    def _open_level(self, price: float) -> tuple[bool, float]:
        ...
        if not self.demo_mode:
            ...
            if available < margin:
                logger.warning(f"Недостаточно маржи: нужно ${margin}, доступно ${available:.2f}. Пропускаем уровень.")
                return False, 0.0
        expected_qty = (margin * LEVERAGE) / price
        ...
        self._save_state()
        logger.info(f"Уровень {self.current_level}: {actual_qty} HYPE @ ${actual_price:.3f} | Ср: ${self.average_price:.3f}")
    ...
    def _check_tp_hit(self, current_price: float) -> tuple[bool, float, float]:
    ...
    def _record_trade(self, profit: float, is_stop: bool, levels: int, now: datetime):
    ...
    def run(self):
        ...
        demo_note = f"\n\n🎮 ДЕМО РЕЖИМ\n💳 Баланс: ${DEMO_BALANCE}" if self.demo_mode else ""
        ...
        while self.running:
            try:
                ...
                if not self.in_trade:
                    ...
                    if not self.demo_mode:
                        ...
                        if pos_size == 0.0:
                            ...
                            if is_stop:
                                ...
                                self.send_keyboard(chat_id, f"⚠️ Позиция закрыта внешним образом\n(SL Bybit или ликвидация)\n\n💸 PnL: ${profit:.2f}\n📊 Уровней было: {levels}")
                            else:
                                self._record_trade(profit, False, levels, now)
                                self.send_keyboard(chat_id, f"✅ TP сработал на Bybit\n\n💰 Прибыль: +${profit:.2f}\n📊 Уровней: {levels}")
                            self._reset()
                            self.recent_high = price
                            continue
                    if self._should_stop(price):
                        ...
                        if self.demo_mode:
                            b = self.bybit.get_balance_info()
                            bal_str = f"\n💳 Баланс: ${b['balance']}"
                        self.send_keyboard(chat_id, f"🔴 СТОП-ЛОСС {self._mode_label()}\n\n💲 Цена: ${price}\n📉 Падение: -{drop}%\n💸 Потеря: ~${loss}\n📊 Уровней было: {levels}\n❌ Стопов всего: {self.stats['total_stops']}\n⏳ Пауза 2 минуты...{bal_str}")
                        self._reset()
                        self.recent_high = price
                        time.sleep(120)
                        continue
                    ...
                    if tp_hit:
                        ...
                        if self.demo_mode:
                            b = self.bybit.get_balance_info()
                            bal_str = f"\n💳 Баланс: ${b['balance']}"
                        self.send_keyboard(chat_id, f"✅ ТЕЙК-ПРОФИТ {self._mode_label()}\n\n💲 Цена выхода: ${tp_price}\n📊 Уровней: {levels}\n⚡ Плечо: {LEVERAGE}x\n💵 Вложено: ${self._total_invested()}\n💰 Прибыль: +${profit}\n\n✅ Сегодня: {self.stats['today_trades']} сделок\n💰 Сегодня: +${round(self.stats['today_profit'], 2)}\n💰 Всего: +${round(self.stats['total_profit'], 2)}{bal_str}\n\n👀 Ищу следующий вход...")
                        self._reset()
                        self.recent_high = price
                        continue
                    averaged = False
                    ...
                    if averaged:
                        ...
                        if self.demo_mode:
                            b = self.bybit.get_balance_info()
                            bal_str = f"\n💳 Баланс: ${b['balance']}"
                        self.send_keyboard(chat_id, f"📉 УСРЕДНЕНИЕ {self._mode_label()} — Уровень {len(self.entries)}\n\n💲 Цена: ${price} (-{drop}%)\n💵 Добавлено: ${MARGINS[self.current_level-1]}\n📊 Вложено: ${self._total_invested()}\n📈 Средняя: ${self.average_price}\n🎯 Новый ТП: ${tp} (+{tp_pct}%)\n📦 HYPE: {self.total_qty}\n⚠️ Осталось уровней: {len(MARGINS) - self.current_level}{bal_str}")
                time.sleep(CHECK_INTERVAL)
                ...
                self.send_keyboard(self._get_chat_id(), f"⚠️ Ошибка:\n{e}\n\nПродолжаю через 30 сек...")
                time.sleep(30)
