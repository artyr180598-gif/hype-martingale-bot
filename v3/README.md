...
### Раннее начало цикла (v3.4)

`analysis/emergence.py` теперь считает две независимые оценки — `long_score`
и `short_score` — по закрытым свечам. В расчёт входят сжатие/база, ускорение
объёма, разворот MACD и RSI, сближение EMA, DI, OBV/CVD и давление закрытий.
Сторона появляется только при достаточном счёте и отрыве от противоположной.

Фазы разделены намеренно:

* `BUILDING` — признаки копятся, входа нет;
* `TURNING` — цикл разворачивается, показывается реальная граница-триггер;
* `CONFIRMED` — закрытая 1h-свеча подтвердила выход, полный движок может
  сформировать LONG/SHORT с зоной входа, стопом и TP;
* `EXHAUSTED` — движение уже прошло слишком далеко, вход блокируется.

Ранний детектор не обходит общий гейт. Авто-push всё равно требует свежие
данные, качество, согласованность анализов, допустимый риск и R:R. Поэтому
наблюдение до пробоя не выдаётся за готовую команду на сделку.
