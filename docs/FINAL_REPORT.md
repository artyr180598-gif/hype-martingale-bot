...
- В этой среде из sandbox все биржи дают TLS/SSL EOF — live-проверка
  невозможна; CLI обрабатывает это дружелюбным предупреждением. Демо-режим
  полностью автономен.
- Bybit не отдаёт публичный REST ликвидаций → используется прокси крупных
  сделок (задокументировано).
