...
class TradeAdvisor:
    ...
    async def build(
        ...
        # Бэктест показал: при зоне дальше ~1.5% от цены лимитник часто не
        # исполняется вовсе (на истории BTC/1h исполнилось только 9% входов).
        # Поэтому предупреждаем раньше и объясняем, что делать, если не вошло.
        if plan.distance_pct > 3.0:
            card.warnings.append(
                f"⚠️ До зоны входа {plan.distance_pct:.1f}% — это далеко. Ставь лимитный ордер "
                f"и НЕ покупай по рынку: все расчёты (стоп, объём, риск) сделаны от цены зоны, "
                f"а не от текущей."
            )
            card.warnings.append(
                "Если ордер не исполнился за 12–24 часа — снимай его. Цена ушла, "
                "сетап устарел, пересчитай план заново, а не догоняй рынок."
            )
        elif plan.distance_pct > 1.5:
            card.warnings.append(
                f"До зоны входа {plan.distance_pct:.1f}% — лимитник может не исполниться. "
                f"Ждать или входить по рынку — твой выбор, но по рынку стоп станет "
                f"ближе, а риск на сделку вырастет."
            )
